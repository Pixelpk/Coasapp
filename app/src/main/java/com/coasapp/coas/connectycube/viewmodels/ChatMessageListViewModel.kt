...
class ChatMessageListViewModel internal constructor(
        ...
        result.addSource(source) {
            if (it.isNullOrEmpty()) {
                result.value = Resource.error(
                    getApplication<Application>().getString(com.coasapp.coas.R.string.error_while_loading_users),
                    null
                )
            } else {
    ...
    fun getOccupantsList(chatDialog: ConnectycubeChatDialog): List<User> {
        val result = MediatorLiveData<Resource<List<ConnectycubeUser>>>()
        result.value = Resource.loading(null)

        val source = usersRepository.getUsersByIdsSync(*chatDialog.occupants.toIntArray())
