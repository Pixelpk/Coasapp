            ...
            if (chatDialog.isPrivate) {
                ...
                /*  AsyncTask<Void, Void, List<User>>() {
                      override fun doInBackground(vararg params: Void?): List<User>? {
                          // ...
                          val users: List<User> = getUserRepository(activityContext).getUsersByIdsSync(*chatDialog.occupants.toIntArray())
                          return users
                      }
                ...
                class UsersTask() : AsyncTask<Void, Void, List<User>>() {
                    ...
                    override fun onPostExecute(result: List<User>?) {
                        super.onPostExecute(result)
                        if (result!!.size > 0) {
                            val usersList: List<User> = APPHelper.userList(activityContext, result);
                            Log.i("DialogUser", Gson().toJson(usersList))
                            if (usersList.size > 0) {
                                loadChatDialogPhoto(
                                        activityContext,
                                        chatDialog.type == ConnectycubeDialogType.PRIVATE,
                                        usersList.get(0).conUser.avatar,
                                        imgAvatar
                                )
                            } else {
                                loadChatDialogPhoto(
